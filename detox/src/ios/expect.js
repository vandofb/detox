...
class ActionInteraction extends Interaction {
  constructor(element, action) {
    ...
    this._call = GreyInteraction.performAction(element._call, action._call);
...
class MatcherAssertionInteraction extends Interaction {
  constructor(element, matcher) {
    ...
    this._call = GreyInteraction.assertWithMatcher(element._call, matcher._call);
...
class WaitForInteraction extends Interaction {
  ...
  async withTimeout(timeout) {
    ...
    let _conditionCall = GreyConditionDetox.detoxConditionForElementMatched(this._element._call())
    if (this._notCondition) {
      _conditionCall = GreyConditionDetox.detoxConditionForNotElementMatched(this._element._call())
    }
...
class WaitForActionInteraction extends Interaction {
  ...
  async _execute(searchAction) {
    ...
    this._call = GreyInteraction.assertWithMatcher(_interactionCall, this._originalMatcher._call);
    await this.execute();
  }
